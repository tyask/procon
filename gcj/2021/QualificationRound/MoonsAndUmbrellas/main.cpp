...
void solve0(int t) {
    INT(X, Y);
    STR(S);
    vec<map<char, int>> dp(S.size()+1);
    erep(S.size()) dp[i]['C'] = dp[i]['J'] = INF;

    if (S[0]=='C') dp[0]['C'] = 0;
    else if (S[0]=='J') dp[0]['J'] = 0;
    else dp[0]['C'] = dp[0]['J'] = 0;

    rep(i, S.size()) {
        if (S[i]=='C') {
            chmin(dp[i+1]['C'], dp[i]['C']);
            chmin(dp[i+1]['J'], dp[i]['C']+X);
        } else if (S[i]=='J') {
            chmin(dp[i+1]['C'], dp[i]['J']+Y);
    ...
    int ans = INF;
    each(p, dp[S.size()]) chmin(ans, p.SE);
