...
#define TEMPLATE(...) overload2(__VA_ARGS__,template2,template1)(__VA_ARGS__)
...
const char* space_or_empty(bool b) { return b ? " " : ""; }
...
TEMPLATE(T) vector<T> cumsum(const vec<T>& v) { vec<T> s(v.size()+1, 0); rep(v.size()) s[i+1] = s[i] + v[i]; return s; }
TEMPLATE(T) vector<T> uniq(const vec<T>& v) { set<T> s(rng(v)); return vec<T>(rng(s)); }
TEMPLATE(T) T pop(queue<T>& q) { T t = q.front(); q.pop(); return t; }
TEMPLATE(T) T pop(priority_queue<T>& q) { T t = q.top(); q.pop(); return t; }

TEMPLATE(N) N gcd(N a, N b){ while(b){ N c = b; b = a % b; a = c; } return a; }
TEMPLATE(Cont) auto gcd(const Cont& c){ using N = typename Cont::value_type; if (c.empty()) return N(0); N g = c[0]; rep(i, 1, c.size()) g = gcd(g, c[i]); return g; }
TEMPLATE(N) N gcd(const initializer_list<N>& ns){ return gcd(vec<N>(ns)); }
TEMPLATE(N) N lcm(N a, N b){ if(!a || !b) return 0; return a / gcd(a, b) * b; }
TEMPLATE(Cont) auto lcm(const Cont& c){ typename Cont::value_type l = 1; rep(c.size()) l = lcm(l, c[i]); return l; }
TEMPLATE(N) N lcm(const initializer_list<N>& ns){ return lcm(vec<N>(ns)); }
