...
#if __ATCODER__ == 1
void solve(long long N, long long H, std::vector<long long> A, std::vector<long long> B, std::vector<long long> C, std::vector<long long> D) {
}
...
void solve() {
    ...
    solve(N, H, std::move(A), std::move(B), std::move(C), std::move(D));
}
#else
void solve() {
}
