...
vec<string> devide(const string& s) {
    if (s.size() == 1) return {s};

    vec<string> r;
    rep(i, 1, s.size()-1) {
        string s1 = string(s.begin(), s.begin()+i);
        string s2 = string(s.begin()+i, s.end());
        if (s1 != s2) {
            vec<string> v(devide(s1));
            each(x, devide(s2)) v.push_back(x);
            if (r.size() < v.size()) {
                r = v;
            }
        }
    ...
    return r.empty() ? vec<string>{s} : r;
}

/*
貪欲法あるいはDPが必要らしい。難しいので一旦保留
*/
void solve() {
    STR(S);
    vec<string> v = devide(S);
    debug(v);
    out(v.size());
}
