...
void solve(ll N, vec<ll> RX, vec<ll> RY, vec<ll> RC, vec<ll> BX, vec<ll> BY, vec<ll> BC) {
}
...
void solve() {
    ...
    vec<ll> RY(N);
    vec<ll> RC(N);
    rep(i, N) { input(RX[i]); input(RY[i]); input(RC[i]); }
    vec<ll> BX(N);
    vec<ll> BY(N);
    vec<ll> BC(N);
    rep(i, N) { input(BX[i]); input(BY[i]); input(BC[i]); }
    solve(N, move(RX), move(RY), move(RC), move(BX), move(BY), move(BC));
}
#else
void solve() {
}
