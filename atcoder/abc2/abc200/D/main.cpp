...
void solve2(ll N, vec<ll> A) {
    ...
    each(p, m) if (p.second.size()>=2) {
        Yes();
        out(p.second[0].size(), p.second[0]);
        out(p.second[1].size(), p.second[1]);
        return;
    }
...
void solve(ll N, vec<ll> A) {
    const int M = 200;
    vvec<ll> dpb(N+1,vec<ll>(M,-1)), dpc(N+1,vec<ll>(M,-1));
    dpb[0][0] = dpc[0][0] = 0;
    rep(i,N) rep(j,M) {
        if(dpb[i][j]>=0) {
            if (dpb[i+1][j]>=0) {
            ...
            if (dpb[i+1][(j+A[i])%M]>=0) {
                // ok
            } else dpb[i+1][j+A[i]%M] = j;
        }
    }
