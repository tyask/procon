...
void solve() {
    ...
    ll cx = max(C);
    vvec<ll> vx(cx+1, vec<ll>(2));
    rep(cx+1) vx[i][0] = inf<ll>, vx[i][1] = -inf<ll>;
    fill(rng(vx[0]), 0);
    rep(N) {
        auto [x, c] = pair{X[i], C[i]};
        chmin(vx[c][0], x);
        chmax(vx[c][1], x);
    }

    // rep(cx) rep(j,2) if(abs(vx[i+1][j])==inf<ll>) {
    //     vx[i+1][j] = vx[i][j];
    // }

    vvec<ll> dp(cx+1, vec<ll>(2, inf<ll>));
    fill(rng(dp[0]), 0);
    rep(i,cx)rep(j,2) {
        if (abs(vx[i+1][j])==inf<ll>) {
            dp[i+1][j] = dp[i][j];
            vx[i+1][j] = vx[i][j];
            continue;
        }

        ll w = vx[i+1][1]-vx[i+1][0];
        chmin(dp[i+1][j],  dp[i][j]+abs(vx[i+1][!j]-vx[i][j]) + w);
        chmin(dp[i+1][!j], dp[i][j]+abs(vx[i+1][j]-vx[i][j]) + w);
    }

    ll ans = inf<ll>;
    rep(j,2) chmin(ans, dp[cx][j]+abs(vx[cx][j]));
    out(ans);
}
#else
void solve() {
