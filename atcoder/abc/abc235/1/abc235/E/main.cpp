...
struct kruskal {
    struct edge {ll u, v, cost, a; };

    int n; // 頂点数
    vec<edge> es;
    vec<bool> ans;

    kruskal(int n, int m) : n(n), ans(m) {}
    ...
    vec<edge> run() {
        ...
        vec<edge> ret;
        each(e, es) {
            if (e.a>=0) ans[e.a] = !uf.same(e.u,e.v);
            else if (uf.unite(e.u, e.v)) ret.PB(e);
        }
        return ret;
    }
};

void solve(ll N, ll M, ll Q, vec<ll> a, vec<ll> b, vec<ll> c, vec<ll> u, vec<ll> v, vec<ll> w) {
    decrements(a,b,u,v);
    kruskal k(N, Q);
    rep(M) k.add({a[i],b[i],c[i],-1});
    rep(Q) k.add({v[i],u[i],w[i],i});
    auto es = k.run();
    each(a, k.ans) Yes(a);
...
void solve() {
    ...
    vec<ll> b(M);
    vec<ll> c(M);
    rep(i,M) { input(a[i]); input(b[i]); input(c[i]); }
    vec<ll> u(Q);
    vec<ll> v(Q);
    vec<ll> w(Q);
    rep(i,Q) { input(u[i]); input(v[i]); input(w[i]); }
    solve(N, M, Q, move(a), move(b), move(c), move(u), move(v), move(w));
}
#else
void solve() {
}
