...
void solve() {
    ...
    auto win = [](int n1, int n2) -> int {
        if (n1 == n2) return 0;

        int m = n1 | n2;
        if (m == (1|2)) return n1 == 1 ? 1 : -1;
        if (m == (2|4)) return n1 == 2 ? 1 : -1;
        if (m == (4|1)) return n1 == 4 ? 1 : -1;
        assert(false);
        return 0;
    };
    ...
    rep(i,M) {
        rep(k,N) {
            int h1 = v[2*k].first;
            int h2 = v[2*k+1].first;
            int r = win(A[h1][i], A[h2][i]);
            if (r>0) v[2*k].second++;
            else if (r<0) v[2*k+1].second++;
        }
        sort(v, comp);
    }
    each(p, v) out(p.first+1);
}
#else
void solve() {
}
