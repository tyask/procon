...
void solve(ll N, vec<ll> X, vec<ll> Y, vec<ll> Z) {
    vec<ll> vis(N);
    vec<ll> d;
    ll ans=0;
    queue<ll> q;
    ...
    while (q.size()) {
        ...
        ll n=-1, mn=inf<ll>;
        rep(N) if(i!=v && !vis[i] && chmin(mn,abs(X[v]-X[i])+abs(Y[v]-Y[i])+abs(Z[v]-Z[i]))) n=i;
        if (n>=0) q.push(n), ans+=abs(X[v]-X[n])+abs(Y[v]-Y[n]);
    }
    out(d);
    ll e = d.back();
    ans+=abs(X[0]-X[e])+abs(Y[0]-Y[e]);
    ans+=max(Z)-min(Z);
    out(ans);
}
...
void solve() {
    ...
    vec<ll> Y(N);
    vec<ll> Z(N);
    rep(i,N) { input(X[i]); input(Y[i]); input(Z[i]); }
    solve(N, move(X), move(Y), move(Z));
}
#else
void solve() {
}
