...
void solve(ll N, ll M, vec<ll> U, vec<ll> V) {
    rep(M) U[i]--, V[i]--;

    UnionFind u(N);
    rep(M) u.unite(U[i], V[i]);

    map<ll, ll> edges;
    rep(M) edges[u.find(U[i])]++;

    mint ans(1);
    for (auto g : u.groups()) {
        if (edges[g[0]] == g.size()) ans *= 2;
        else ans = 0;
    }
    out(ans);
}
...
void solve() {
    ...
    vec<ll> U(M);
    vec<ll> V(M);
    rep(i, M) { in(U[i]); in(V[i]); }
    solve(N, M, move(U), move(V));
}
#else
void solve() {
}
