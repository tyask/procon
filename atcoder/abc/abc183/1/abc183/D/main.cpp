...
void solve(ll N, ll W, vec<ll> S, vec<ll> T, vec<ll> P) {
    const ll M = 2*pow10(5)+10;
    vec<ll> v(M);
    rep(N) v[S[i]]+=P[i],v[T[i]]-=P[i];
    // out(max(v));
    Yes(max(cumsum(v))<=W);
}
...
void solve() {
    ...
    vec<ll> T(N);
    vec<ll> P(N);
    rep(i,N) { input(S[i]); input(T[i]); input(P[i]); }
    solve(N, W, move(S), move(T), move(P));
}
#else
void solve() {
}
