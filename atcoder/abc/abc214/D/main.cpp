...
void solve(ll N, vec<ll> u, vec<ll> v, vec<ll> w) {
    rep(N-1) u[i]--, v[i]--;
    struct we { ll v, w; };
    vvec<we> g(N);
    rep(N-1) g[u[i]].PB({v[i],w[i]}), g[v[i]].PB({u[i],w[i]});

    vec<ll> dp(N), vis(N);
    auto dfs = [&](auto self, ll v, ll mx) {
        if (vis[v]) return;
        vis[v]=1;

        dp[v]=mx;
        each(n,g[v]) self(self, n.v, max(mx, n.w));
        return;
    };
...
void solve() {
    ...
    vec<ll> v(N-1);
    vec<ll> w(N-1);
    rep(i, N-1) { in(u[i]); in(v[i]); in(w[i]); }
    solve(N, move(u), move(v), move(w));
}
#else
void solve() {
}
