...
void solve() {
    ...
    unionfind uf(N);
    rep(M) uf.unite(A[i],B[i]);

    vec<ll> vis(N), col(N);
    auto dfs = [&](auto self, ll v, const vec<ll>& gr) -> ll {
        out(v, ":", col);
        vis[v]=1;

        set<ll> s{1,2,3};
        each(n, g[v]) s.erase(col[n]);
        if (s.empty()) return 0;

        bool reach=all_of(rng(gr), [&](ll x){return col[x]>0;});
        if (reach) {
            out(v, ":", col);
            return 1;
        }
