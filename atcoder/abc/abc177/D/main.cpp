...
void solve2(long long N, long long M, std::vector<long long> A, std::vector<long long> B) {
    dsu d(N);
    rep(M) {
        d.merge(A[i]-1, B[i]-1);
    }
...
void solve(long long N, long long M, std::vector<long long> A, std::vector<long long> B) {
    vvec<ll> f(N);
    rep(M) {
        f[A[i]-1].push_back(B[i]-1);
        f[B[i]-1].push_back(A[i]-1);
    }
...
void solve() {
    ...
    solve2(N, M, std::move(A), std::move(B));
}
#else
void solve() {
}
