...
void solve() {
    LL(N);
    const ll M = 2*N;
    vvec<ll> A(M-1, vec<ll>(M));
    rep(i,M-1) rep(j,M-i-1) cin>>A[i][i+j+1];
    ...
    auto dfs = [&](auto self, ll n) -> ll {
        if (n==N) {
            ...
            rep(M) pp[v[i]].PB(i);
            ll a=0;
            each(p,pp) a^=A[p[0]][p[1]];
            return a;
        }

        ll r=0;
        ll si=find_if(rng(v),[](int n){return n<0;})-v.begin();
        v[si]=n;
        rep(i,si+1,M) if(v[i]<0) {
            v[i]=n;
            chmax(r, self(self,n+1));
            v[i]=-1;
        }
        v[si]=-1;
        return r;
