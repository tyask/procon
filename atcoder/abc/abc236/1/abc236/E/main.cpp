...
void solve(ll N, vec<ll> A) {
    ...
    ld avg=0,sum=0,cnt=0;
    vec<ll> ax(N);
    rep(N) {
        ll pi=p[i].SE, ni=pi+1;
        sum+=p[i].FI;
        cnt++;
        ld cavg=sum/cnt;
        if (ni==N || ax[ni]==0) ax[pi]=1;
        else if(cavg>avg) ax[pi]=1, avg=cavg;
        else break;
    }
    ...
    rep(N) {
        mp.PB(p[i].FI);
        ll cmn=mp[(mp.size()+1)/2];
        ll pi=p[i].SE, ni=pi+1;
        if (ni==N || ax[ni]==0) mx[pi]=1;
        else if(cmn>mn) mx[pi]=1, mn=cmn;
        else break;
    }
