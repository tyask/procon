...
void solve() {
    ...
    auto g = graph<ll>(N).digraph(X,Y);
    auto h = graph<ll>(N).digraph(Y,X);

    vec<ll> sm(N,-inf<ll>);
    auto sdfs = [&](auto self, ll v, ll m) {
        if(!chmax(sm[v],max(A[v],m))) return;
        each(n,h[v]) self(self,n,sm[v]);
    };
    rrep(N) sdfs(sdfs,i,A[i]);

    ll ans=-inf<ll>;
    rep(N) each(n,g[i]) chmax(ans,sm[n]-A[i]);
    out(ans);
}
#else
void solve() {
