...
void solve() {
    ...
    while(q.size()) {
        auto[w,i,j]=pop(q);
        out(w,i,j);
        if (gw[i][j]<w) continue;
        gw[i][j]=L544509
        
        rep(k,4) {
            auto[ni,nj]=dyx(i,j)[k];
            if (in(g,ni,nj)&&S[ni][nj]=='.'&&g[ni][nj]<g[i][j]+1) {
                q.emplace(gw[i][j],ni,nj);
            }
        }

        erep(di,-2,2) erep(dj,-2,2) if(di!=0 && dj!=0) {
            auto[ni,nj]=pair{i+di, j+dj};
            if(in(g,ni,nj)&&S[ni][nj]=='.'
                    && g[ni][nj]<g[i][j]+1
                    && gw[ni][nj]<gw[i][j]+1
                    ) {
                q.emplace(gw[i][j],ni,nj);
