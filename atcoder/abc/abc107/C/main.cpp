...
#if __ATCODER__ == 1
void solve2(long long N, long long K, std::vector<long long> x) {
    int ans = INF;
    erep(l, N-K) {
        int r = l + K - 1;
        int lv = x[l];
        int rv = x[r];
        chmin(ans, abs(0-lv) + abs(lv-rv));
        chmin(ans, abs(0-rv) + abs(lv-rv));
    }
...
void solve(long long N, long long K, std::vector<long long> x) {
    auto it = lower_bound(rng(x), 0);
    vec<int> neg(x.begin(), it);
    reverse(rng(neg));
    vec<int> pos(it, x.end());
    int ans = INF;
    int rm = min<int>(K, pos.size());
    erep(r, rm) {
        int l = K - r;
        if (l > neg.size()) continue;
        int lp = (l == 0) ? 0 : -neg[l-1];
        int rp = (r == 0) ? 0 : pos[r-1];
        chmin(ans, min(lp, rp) * 2 + max(lp, rp));
    }
