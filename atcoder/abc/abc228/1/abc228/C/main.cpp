...
TEMPLATE(Cont) auto sort(Cont& c) { std::sort(rng(c)); }
...
void solve(ll N, ll K, vec<vec<ll>> P) {
    vec<ll> s(N);
    rep(N)  s[i] = sum(P[i]);
    sort(s, greater<ll>());
    rep(N) {
        ll t = sum(P[i]);
        t += 300;
        ll a = lower_bound(rng(s), t, greater<ll>()) - s.begin();
        Yes(a+1<=K);
    }
}
...
void solve() {
    LL(N);
    LL(K);
    vec<vec<ll>> P(N, vec<ll>(3));
    rep(i, N) rep(j, 3) { input(P[i][j]); }
    solve(N, K, move(P));
}
#else
void solve() {
