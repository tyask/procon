...
void solve(ll N, ll K) {
    auto g1 = [&](ll x){ str s = to_string(x); return stoll(sort(s, greater<char>())); };
    auto g2 = [&](ll x){ str s = to_string(x); return stoll(sort(s, less<char>())); };
    auto f  = [&](ll x) { return g1(x) - g2(x); };
