...
struct dijkstra {
    ...
    void run(int s) {
        ...
        while (!que.empty()) {
            ...
            for (edge e : g[v]) {
                if (chmin(d[e.to], ceil(d[v],e.k)*e.k+e.cost)) {
...
void solve(ll N, ll M, ll X, ll Y, vec<ll> A, vec<ll> B, vec<ll> T, vec<ll> K) {
    X--,Y--;
    decrements(A,B);
    dijkstra d(N);
    rep(M) d.add(A[i], {B[i],T[i],K[i]}), d.add(B[i],{A[i],T[i],K[i]});
    d.run(X);
    if (d[Y]!=inf<ll>) out(d[Y]); else out(-1);
}
...
void solve() {
    ...
    vec<ll> T(M);
    vec<ll> K(M);
    rep(i, M) { input(A[i]); input(B[i]); input(T[i]); input(K[i]); }
    solve(N, M, X, Y, move(A), move(B), move(T), move(K));
}
#else
void solve() {
}
