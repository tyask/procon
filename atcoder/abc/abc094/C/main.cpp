...
void solve(long long N, std::vector<long long> X) {
    vec<pair<int, ll>> x(N);
    rep(N) x[i] = {i, X[i]};
    sort(x, [](auto p1, auto p2){ return p1.second < p2.second; });
    ...
    rep(N) {
        int pos = distance(x.begin(), lower_bound(rng(x), make_pair(i, X[i]), comp));
        out(pos<N/2? x[N/2].second : x[N/2-1].second);
    }
}
