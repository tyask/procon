...
TEMPLATE(T)    auto sumproduct(const initializer_list<T>& c) { return sumproduct(vec<T>(c)); }
...
bool comcon(pii p1, pii p2) {
    return p1.second * (p2.first + p2.second) <= p2.second * (p1.first + p1.second);
}
...
void solve(long long A, long long B, long long C, long long D, long long E, long long F) {
    ...
    erep(F) {
        ...
        if (i+A <= F && comcon(v[i+A], {a+A, b}) && can(a+A, b)) v[i+A] = {a+A, b};
        if (i+B <= F && comcon(v[i+B], {a+B, b}) && can(a+B, b)) v[i+B] = {a+B, b};
        if (i+C <= F && comcon(v[i+C], {a, b+C}) && can(a, b+C)) v[i+C] = {a, b+C};
        if (i+D <= F && comcon(v[i+D], {a, b+D}) && can(a, b+D)) v[i+D] = {a, b+D};
    }

    pii ans;
    erep(F) if (comcon(ans, v[i])) ans = v[i];
    out(ans.first+ans.second, ans.second);
}

void solve2(long long A, long long B, long long C, long long D, long long E, long long F) {
    ::E = E;
    set<int> ws, ss;
    rep(a, 30) rep(b, 30) {
        int v = a*100*A+b*100*B;
        if (v <= F) ws.insert(v);
    }

    rep(c, 3000) rep(d, 3000) {
        int v = c*C + d*D;
        if (v <= F) ss.insert(v);
    }
    ...
    each(w, ws) {
        each(s, ss) {
            if (w+s > F) break;
            if (can(w, s) && comcon(ans, {w, s})) ans = {w, s};
        }
    }
