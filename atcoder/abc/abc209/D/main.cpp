...
void solve() {
    LL(N, Q);
    vec<ll> a(N-1), b(N-1), c(Q), d(Q);
    rep(N-1) cin >> a[i] >> b[i], a[i]--, b[i]--;
    rep(Q) cin >> c[i] >> d[i], c[i]--, d[i]--;

    vvec<ll> g(N);
    dijkstra dk(N);
    rep(N-1) {
        dk.add((int)a[i], {(int) b[i], 1LL});
        dk.add((int)b[i], {(int) a[i], 1LL});
        g[a[i]].PB(b[i]);
        g[b[i]].PB(a[i]);
    }
    ...
    map<pll, ll> m;
    rep(Q) {
        pll t{min(c[i],d[i]),max(c[i],d[i])};
        if (m[t]) continue;
        dk.run(t.first);
        rep(i, dk.d.size()) {
            m[{t.first, i}] = m[{i,t.first}] = dk.d[i];
        }
    }
    rep(Q) {
        if(m[{min(c[i],d[i]),max(c[i],d[i])}]%2==0) out("Town");
        else out("Road");
    }
}
#else
