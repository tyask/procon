...
void solve() {
    ...
    x2-=x0, y2-=y0;
    double x = x2/2.0, y = y2/2.0;
    double th = -PI*(N-2)/N/2;
    double ax = x*cosf64(th)-y*sinf64(th);
    double ay = x*sinf64(th)+y*cosf64(th);
    double al = sqrt(ax*ax+ay*ay);

    double r = sqrt(x*x+y*y);
    double l = sqrt(r*r+r*r-2*r*r*cosf64(2*PI/N));

    out(ax*l/al+x0, ay*l/al+y0);
}
