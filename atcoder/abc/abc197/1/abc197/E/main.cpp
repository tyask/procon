...
void solve() {
    ...
    pll ex = {inf<ll>, -inf<ll>};
    vec<pll> m(N, ex);
    rep(N) chmin(m[C[i]].FI,X[i]), chmax(m[C[i]].SE,X[i]);
    ...
    rep(N) if(m[i]!=ex) {
        ...
        auto [c1,c2] = m[i];
        ll d = abs(c2-c1);
        dist.FI=min(d1+abs(p1-c2)+d, d2+abs(p2-c2)+d);
        dist.SE=min(d1+abs(p1-c1)+d, d2+abs(p2-c1)+d);
        pre=m[i];
    }

    dist.FI+=abs(pre.FI);
    dist.SE+=abs(pre.SE);
    out(min(dist.FI,dist.SE));
}
#else
void solve() {
}
