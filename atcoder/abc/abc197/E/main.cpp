...
void solve() {
    ...
    rep(N) cin >> X[i] >> C[i];
    map<int, vec<ll>> m;
    rep(N) m[C[i]].push_back(X[i]);

    vec<ll> dist(2);
    vec<ll> pos(2);
    each(pi, m) {
        vec<ll> predist(2, LINF), prepos(2);
        swap(predist, dist);
        swap(prepos, pos);
        auto& v = pi.SE;
        ll mn = min(v), mx = max(v), l = mx - mn;

        rep(2) {
            chmin(dist[1], abs(prepos[i]-mn) + l + predist[i]);
            chmin(dist[0], abs(prepos[i]-mx) + l + predist[i]);
        }
        pos = {mn, mx};
    }

    rep(2) dist[i] += abs(pos[i]);
