...
void solve(long long N, long long M, std::vector<long long> X) {
    ...
    sort(rng(X));
    vec<pair<int, ll>> s(M-1);
    rep(i, 0, M-1) s[i] = {i, X[i+1]-X[i]};
    sort(rng(s), [](auto p1, auto p2){ return p1.second > p2.second; });
    vec<pair<int, ll>> s2(rng(s, N-1));
    sort(rng(s2), [](auto p1, auto p2){ return p1.first < p2.first; });
    s2.push_back({M-1, 0});
    ll ans = 0;
    int prev = 0;
    each(p, s2) {
        ans += X[p.first] - X[prev];
        prev = p.first+1;
    }
    out(ans);
}
