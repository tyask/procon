...
void solve(ll N, ll M, vec<ll> H, vec<ll> U, vec<ll> V) {
    decrements(U,V);
    dijkstra d(N);
    rep(M) {
        auto [u,v]=binder(U,V)[i];
        if (H[u]<H[v]) swap(u,v);
        ll hu=H[u], hv=H[v];
        // ポテンシャルとして、実際のcostに+Hx,-Hyすることで、costが正になるように調整する.
        // (Hx>Hy)
        //    Hx+(Hx-Hy)-Hy=0
        //    ---------------------->
        // Hx                         Hy
        //    <---------------------
        //    Hy+2*(Hx-Hy)-Hx=Hx-Hy
        d.add(u,{v,hu-(hu-hv)-hv});
        d.add(v,{u,hv+2*(hu-hv)-hu});
    }

    d.run(0,0);
    ll ans=0;
    rep(N) chmax(ans, -(d[i]-H[0]+H[i]));
    out(ans);
}
...
void solve() {
    ...
    vec<ll> U(M);
    vec<ll> V(M);
    rep(i,M) { input(U[i]); input(V[i]); }
    solve(N, M, move(H), move(U), move(V));
}
#else
void solve() {
}
