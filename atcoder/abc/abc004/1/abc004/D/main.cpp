...
void solve(ll R, ll G, ll B) {
    ...
    ll ans = sumad<ll>(p, 1, 1) + sumad<ll>(n, 1, 1);
    // out(ans);
    if (p >= 100) {
        ans += sumad<ll>(B, p-100+1, 1);
    } else {
        ll d = 99-n;
        ans += sumad<ll>(min<ll>(d, (B-1)/2), 1, 1);
        ans += sumad<ll>(B/2 + max<ll>(0, (B-1)/2-d), 1, 1);
    }
    // out(ans);

    if (n >= 100) {
        ans += sumad<ll>(R, n-100+1, 1);
    } else {
        ll d = 99-n;
        ans += sumad<ll>(min<ll>(d, (R-1)/2), 1, 1);
        ans += sumad<ll>(R/2 + max<ll>(0, (R-1)/2-d), 1, 1);
    }
