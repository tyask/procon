...
int dfs(int n, int c) {
    ...
    if (ng.count(n) > 0) return dp[n][c] = 0;
    if (n == 0) return dp[n][c] = 1;
    erep(i, 1, 3) {
        if (dfs(n-i, c-1)) return dp[n][c] = 1;
    }
...
void solve(long long N, std::vector<long long> NG) {
    ng.insert(rng(NG));
    dp.resize(N+1, vec<int>(100+1, -1));
    dfs(N, 100);
    YES(dp[N][100]);
}

void solve2(long long N, std::vector<long long> NG) {
    vec<int> ng(300+1);
    rep(NG.size()) ng[NG[i]]++;
    vec<int> dp(N+1, INF);
    dp[0] = 0;
    erep(i, 0, N) {
        erep(j, 1, 3) {
            if (i+j <= N && !ng[i+j]) chmin(dp[i+j], dp[i]+1);
        }
    }
    YES(dp[N]<=100);
}
