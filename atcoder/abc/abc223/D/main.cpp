...
void solve(ll N, ll M, vec<ll> A, vec<ll> B) {
    vvec<ll> g(N);
    rep(M) g[A[i]-1].push_back(B[i]-1);

    set<ll> vis, ok;
    auto dfs = [&](auto self, ll x) -> bool {
        if (ok.find(x) != ok.end()) return true;
        if (!vis.insert(x).second) return false;
        each(n, g[x]) if (!self(self, n)) return false;
    ...
    set<pll> m;
    rep(M) m.emplace(A[i], B[i]);
    
    // a < b
    auto comp = [&](ll a, ll b) {
        auto it1 = m.find({a, b});
        auto it2 = m.find({b, a});
        if (it1 == m.end() && it2 == m.end()) return a < b;
        if (it1 != m.end()) return true;
...
void solve2(ll N, ll M, vec<ll> A, vec<ll> B) {
    vvec<ll> g(N);
    vec<ll> deg(N);
    rep(M) {
        auto [a, b] = pair{A[i]-1, B[i]-1};
        g[a].push_back(b);
        deg[b]++;
    }
    ...
    while (q.size()) {
        ll v = pop(q);
        ans.push_back(v+1);
        each(n, g[v]) if (--deg[n]==0) q.push(n);
    }

    if (ans.size()==N) out(ans);
    else out(-1);
}
...
void solve() {
    ...
    vec<ll> A(M);
    vec<ll> B(M);
    rep(i, M) { in(A[i]); in(B[i]); }
    solve2(N, M, move(A), move(B));
}
#else
void solve() {
}
