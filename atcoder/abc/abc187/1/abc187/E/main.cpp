...
void solve(ll N, vec<ll> a, vec<ll> b, ll Q, vec<ll> t, vec<ll> e, vec<ll> x) {
    decrements(a,b,e);
    auto g=graph(N).undigraph(a,b);
    LCA lca(g);

    vec<ll> s(N);
    rep(Q) {
        auto [ti,ei,xi] = binder(t,e,x)[i];
        auto [ai,bi] = binder(a,b)[ei];
        ll p = lca.query(ai,bi);
        ll c = (ai+bi) - p;
    ...
    vec<ll> vis(N), ans(N);
    auto dfs = [&](auto self, auto v, ll m) {
        if (vis[v]) return;
        vis[v]=1;
        ans[v]=m+s[v];
        each(n,g[v]) self(self, n, ans[v]);
    };
...
void solve() {
    ...
    vec<ll> a(N-1);
    vec<ll> b(N-1);
    rep(i,N-1) { input(a[i]); input(b[i]); }
    LL(Q);
    vec<ll> t(Q);
    vec<ll> e(Q);
    vec<ll> x(Q);
    rep(i,Q) { input(t[i]); input(e[i]); input(x[i]); }
    solve(N, move(a), move(b), Q, move(t), move(e), move(x));
}
#else
void solve() {
}
