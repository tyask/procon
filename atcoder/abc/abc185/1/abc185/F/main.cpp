...
struct segtree {
    ...
    void apply(ll i, N v) { update(i, fx((*this)[i],v)); }
...
void solve() {
    ...
    VEC(ll,A,N);
    vec<ll> T(Q),X(Q),Y(Q);
    rep(Q) cin>>T[i]>>X[i]>>Y[i];

    segtree seg(N, [](ll x, ll y){return x^y;}, 0);
    rep(N) seg.update(i,A[i]);

    rep(Q) {
        auto[t,x,y]=binder(T,X,Y)[i];
        if(t==1) seg.apply(x-1,y);
        else out(seg(x-1,y));
    }
}
#else
