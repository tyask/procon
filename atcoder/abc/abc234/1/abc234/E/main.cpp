...
void solve(ll X) {
    set<ll> s;
    auto dfs = [&](auto self, ll x, ll d) {
        s.insert(x);
        ll d1 = x%10;
        if (d1+d<0 || d1+d>=10 || ndigits(x)>=ndigits(X)+1) return;
        self(self, x*10+(x%10)+d, d);
    };
    rep(i,1,10)erep(d,-9,9) dfs(dfs, i, d);
    out(*s.lower_bound(X));
}
...
int main() {
    auto solver = [&]() {  try { solve(); } catch (const exit_silently& e) {} };
#if __TEST__
    rep(10) {
        solver();
        out("*****");
