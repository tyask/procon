...
void solve(ll K, str S, str T) {
    vec<ll> tc(10), ac(10);
    rep(4) tc[S[i]-'0']++, ac[T[i]-'0']++;

    ll tp=0, ap=0;
    rep(i,1,10) tp+=i*pow10(tc[i]), ap+=i*pow10(ac[i]);

    vec<ll> xc(10);
    rep(i,1,10) xc[i]=K-(tc[i]+ac[i]);

    ll tx=0;
    erep(i,1,9) erep(j,1,9) {
        if (i==j && tc[i]+ac[i]>K-2) continue;
        if ((i!=j) && (tc[i]+ac[i]>K-1 || tc[j]+ac[j]>K-1)) continue;

        ll tp2 = tp - i*pow10(tc[i]) + i*pow10(tc[i]+1);
        ll ap2 = ap - j*pow10(ac[j]) + j*pow10(ac[j]+1);
        if (tp2>ap2) { if (i==j) tx+=xc[i]*(xc[j]-1); else tx+=xc[i]*xc[j]; }
    }
