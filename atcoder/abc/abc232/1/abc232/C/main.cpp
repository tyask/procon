...
void solve(ll N, ll M, vec<ll> A, vec<ll> B, vec<ll> C, vec<ll> D) {
    vvec<ll> g1(N, vec<ll>(N));
    vvec<ll> g2(N, vec<ll>(N));
    decrements(A, B, C, D);
    rep(M) g1[A[i]][B[i]] = g1[B[i]][A[i]] = 1;
    rep(M) g2[C[i]][D[i]] = g2[D[i]][C[i]] = 1;
...
void solve() {
    ...
    vec<ll> C(M);
    vec<ll> D(M);
    rep(i, M) { input(C[i]); input(D[i]); }
    solve(N, M, move(A), move(B), move(C), move(D));
}
#else
void solve() {
}
