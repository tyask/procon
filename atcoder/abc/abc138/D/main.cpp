...
void solve2(long long N, long long Q, std::vector<long long> a, std::vector<long long> b, std::vector<long long> p, std::vector<long long> x) {
    g.resize(N);
    ans.resize(N);
...
void solve() {
    ...
    solve2(N, Q, std::move(a), std::move(b), std::move(p), std::move(x));
}
#else
void solve() {
}
