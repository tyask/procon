...
void solve(ll N, vec<ll> C, vec<ll> A, vec<ll> B) {
    decrements(A,B);
    graph g = graph(N).undigraph(A,B);

    vec<ll> vc(100001), ans(N);
    auto dfs = [&](auto self, ll v, ll p) -> void {
        int c = C[v];
        if (!vc[c]) ans[v]=1;

        vc[c]++;
        each(n, g[v]) if (n!=p) self(self, n, v);
        vc[c]--;
    };

    dfs(dfs, 0, -1);
    rep(N) if(ans[i]) out(i+1);
}
