...
void solve(vec<str> S) {
    set<char> cs;
    rep(3) each(c,S[i]) cs.insert(c);
    if (cs.size()>10) out("UNSOLVABLE").exit();
    ...
    do {
        map<char,int> m;
        rep(sz(cc)) m[cc[i]]=ns[i];

        vec<ll> nx(3);
        rep(3) each(c, S[i]) nx[i] = nx[i]*10 + m[c];
        bool ok = true;
        rep(3) if(ndigits(nx[i])!=S[i].size() || nx[i]==0) ok=false;
        if (ok && nx[0]+nx[1]==nx[2]) {
            rep(3) out(nx[i]);
            return;
        }

    } while (next_permutation(rng(ns)));
