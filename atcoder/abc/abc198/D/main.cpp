...
ll to(const str& s, const map<char, int>& m) {
    if (m.at(s[0])==0) return 0;
    ll d = 0;
    rep(s.size()) d = d * 10 + m.at(s[i]);
    return d;
}

void solve(vec<str> S) {
    set<char> cs;
    each(s, S) each(c, s) cs.insert(c);
    if (cs.size()>10) {
        out("UNSOLVABLE");
        return;
    }
    ...
    map<char, int> m;
    vec<int> ds;
    rep(10) ds.push_back(i);
    bool ans = false;
    ll n1, n2, n3;
    do {
        rep(vc.size()) m[vc[i]] = ds[i];
        n1 = to(S[0], m);
        n2 = to(S[1], m);
        n3 = to(S[2], m);
