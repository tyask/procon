...
void solve(vec<ll> r, vec<ll> c) {
    ll r1=r[0],r2=r[1],c1=c[0],c2=c[1];
    ll dr=abs(r1-r2), dc=abs(c1-c2);
    ll rx=r2-r1, cx=c2-c1;
    out(r1,c1,r2,c2,dr,dc);
    if (r1==r2&&c1==c2)       out(0).exit();
    if (dr==dc || dr+dc<=3)   out(1).exit();
    if ((r1+c1)%2==(c1+c2)%2) out(2).exit();
    if ((-cx-3<=rx&&rx<=-cx+3) || (cx-3<=rx&&rx<=cx+3)) out(2).exit();
    out(3);
}
