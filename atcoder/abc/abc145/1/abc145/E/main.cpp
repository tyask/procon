...
void solve(ll N, ll T, vec<ll> A, vec<ll> B) {
    vvec<ll> dp(N+1, vec<ll>(T));
    vvec<ll> dist(N+1, vec<ll>(T));
    rep(i,N) {
        ll t = A[i], v = B[i];
        rep(j,T) {
            dp[i+1][j] = dp[i][j];
        ...
        rep(j,T) {
            if (j+t<T-1) if (chmax(dp[i+1][j+t], dp[i][j]+v)) dist[i+1][j+t] = j;
        }
    }
...
void solve() {
    ...
    vec<ll> A(N);
    vec<ll> B(N);
    rep(i, N) { input(A[i]); input(B[i]); }
    solve(N, T, move(A), move(B));
}
#else
void solve() {
}
