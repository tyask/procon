...
#define DBL(...)  ld     __VA_ARGS__; input(__VA_ARGS__)
...
void solve(ld X, ld Y, ld R) {
    const ll M = 10000;
    const ll x = round(X*M);
    const ll y = round(Y*M);
    const ll r = round(R*M);

    auto floor = [&](ll a){return a>=0? a/M*M : (a-(M-1))/M*M; };

    ll ans=0;
    erep(xi,floor(x-r),floor(x+r),M) {
        if (xi<x-r) continue;
        ll dx = abs(xi-x);
        ll dy = r*r-dx*dx;

        ll u = bin_search<ll>(y+r+1, y-1, [&](ll a){return (a-y)*(a-y)<=dy;});
        ll b = bin_search<ll>(y-r-1, y+1, [&](ll a){return (a-y)*(a-y)<=dy;});
        ll ul = (u>=0) ? u/M : (u-(M-1))/M;
        ll bl = (b>=0) ? (b+(M-1))/M : b/M;
        // out(xi,u,b,ul,bl,dx,dy);
        ans+=max(0LL,ul-bl+1);
    }
    out(ans);
}
