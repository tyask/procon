...
void solve(ll N, vec<str> S) {
    ...
    auto dfs = [&](auto self, ll i, ll v) -> ll {
        ...
        ll ret;
        if (v) {
            if (a) ret=self(self,i-1,1);
            else ret=self(self,i-1,1)*2 + self(self,i-1,0);
        } else {
            if (a) ret=self(self,i-1,0)*2 + self(self,i-1,1);
            else ret=self(self,i-1,0);
        }
        return m[i][v]=ret;
    };
