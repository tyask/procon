...
void solve(long long N, std::vector<long long> A) {
    ...
    vec<ll> cs  = cumsum(A);
    vec<ll> cs2 = cumsum(a2);

    ll ans = 0;
    rep(i, 1, N) {
        ll a = A[i];
        ans += a*a*i - 2*cs[i]*a + cs2[i];
    }

    out(ans);
}

void solve2(long long N, std::vector<long long> A) {
    ll s = 0, s2 = 0;
    ll ans = 0;
    rep(i, 1, N) {
        s  += A[i-1];
...
void solve3(long long N, std::vector<long long> A) {
    const int M = 200;
    vec<int> m(M*2+1);
    rep(N) m[A[i]+M]++;
    ll ans = 0;
    rep(i, M*2+1) rep(j, i+1, M*2+1) {
        ll s = i - j;
        ans += m[i]*m[j]*s*s;
    }
    out(ans);
