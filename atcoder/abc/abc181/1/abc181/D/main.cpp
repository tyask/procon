...
void solve(str S) {
    if (sz(S)==1) {
        ...
        str r = S;
        reverse(r);
        Yes(stoll(S)%8==0 || stoll(r)%8==0);
    } else {
        map<int, int> m;
        vec<char> v;
        each(c,S) if (++m[c-'0']<=3) v.PB(c);

        auto dfs = [&](auto self, str s, ll k) -> bool {
            if (s.size()==3) {
                sort(s);
                do {
                    ll n = stoll(s);
                    if (n>=100 && n%8==0) return true;
                } while (next_permutation(rng(s)));
                return false;
            }
            rep(i,k,sz(v)) if(self(self, s+v[i],i+1)) return true;
            return false;
        };
