...
void solve(ll N, ll K, vec<str> S) {
    auto enc = [&](ll i, ll j) { return i*N+j; };

    set<ll> m;
    set<set<ll>> ss;
    auto dfs = [&](auto self, ll pos, ll pre) {
        ll ci = pos/N;
        ll cj = pos%N;
        if (!in(S, ci, cj)||S[ci][cj]=='#') return;
        ...
        each(b, m) {
            rep(k,4) {
                auto [bi, bj] = pair{b/N+dy[k], b%N+dx[k]};
                ll npos = enc(bi, bj);
                if (npos != pre) self(self, npos, pos);
            }
        }
    ...
    rep(i,N)rep(j,N) dfs(dfs, enc(i,j), -1);
    out(ss.size());
}

void solve2(ll N, ll K, vec<str> S) {
    ll ans = 0;
    auto dfs = [&](auto self, vec<str> s) {
        // rep(N) out(s[i]);
        // out("-----");
        int cnt = 0;
        ...
        rep(i,N)rep(j,N) if(s[i][j]=='.') {
            if (cnt != 0) {
                ...
                rep(k,4) {
                    ...
                    if (in(s, ni, nj) && s[ni][nj]=='o') ok = true;
                }
                if (!ok) continue;
            }
