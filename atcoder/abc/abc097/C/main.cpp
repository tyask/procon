...
#if __ATCODER__ == 1
void solve2(std::string s, long long K) {
    set<string> ss;
    rep(pos, s.size()) {
        erep(len, 1, K) {
            if (pos + len <= s.size())
                ss.insert(s.substr(pos, len));
        }
    }

    out(vec<string>(rng(ss))[K-1]);
}

void solve(std::string s, long long K) {
    map<char, vec<int>> m;
    rep(s.size()) m[s[i]].push_back(i);
        ...
        erep(len, 1, n) {
            each(pos, m[c]) {
                if (pos + len <= s.size()) {
                    ...
                    if (distance(ss.begin(), ss.lower_bound(sub)) < K) {
                        ss.insert(sub);
                    }
                }
            }
