...
void solve() {
    ...
    vec<pii> vs(N);
    vec<int> X(M);
    rep(N) cin >> vs[i].second >> vs[i].first;
    rep(M) cin >> X[i];
    sort(vs, greater<pii>());
    rep(q, Q) {
        INT(L, R); L--; R--;
        priority_queue<int, vec<int>, greater<int>> que;
        priority_queue<int, vec<int>, greater<int>> que2;
        rep(i, 0, L)   que2.push(X[i]);
        rep(i, R+1, M) que2.push(X[i]);
        ...
        each(p, vs) {
            ...
            while (!que.empty()) que2.push(pop(que));
        }
