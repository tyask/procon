...
struct range { ll l, r, t; };
inline bool operator<(range r1, range r2) {
    if (r1.l != r2.l) return r1.l < r2.l;
    if (r1.r != r2.r) return r1.r < r2.r;
    if (r1.t != r2.t) return r1.t < r2.t;
    return false;
}
...
void solve() {
    LL(N);
    vec<range> rr(N);
    rep(N) cin >> rr[i].t >> rr[i].l >> rr[i].r;

    ll ans = 0;
    rep(i,N) rep(j,i+1,N) {
        range r1 = min({rr[i], rr[j]});
        range r2 = max({rr[i], rr[j]});
        if (r2.l < r1.r) ans++;
        else if (r2.l == r1.r) {
            if ((r1.t == 1 || r1.t == 3) && (r2.t == 1 || r2.t == 2)) ans++;
        }
    }
    out(ans);
}
...
void solve2() {
    ...
    rep(N) {
        ...
        pp[i].first *= 2;
        pp[i].second *= 2;
        if (t==2||t==4) pp[i].second--;
        if (t==3||t==4) pp[i].first++;
    }

    ll ans = 0;
    rep(i,N) rep(j,i+1,N) {
        ans += max({pp[i].first, pp[j].first}) <= min({pp[i].second, pp[j].second});
    }
    out(ans);
}
