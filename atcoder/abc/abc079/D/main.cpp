...
vec<int> m(10, INF);
int mask = 1 << 10 - 1;
void dfs(int f, int v, int c, int b) {
    if ((1<<v)&b) return;
    b |= (1<<v);
...
void solve2(long long H, long long W, std::vector<std::vector<long long>> c, std::vector<std::vector<long long>> A) {
...
void solve() {
    ...
    solve2(H, W, std::move(c), std::move(A));
}
#else
void solve() {
}
