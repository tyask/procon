...
void solve(ll N, vec<vec<ll>> D) {
    vec<ll> d;
    rep(i,N) d.PB(D[i][0]==D[i][1]);
    auto r = runlength_encoding(d);
    bool ans=false;
    each(p,r) if(p.FI==1 && p.SE>=3) ans=true;
    Yes(ans);
}

void solve() {
    LL(N);
    vec<vec<ll>> D(N,vec<ll>(2));
    rep(i,N) rep(j,2) { input(D[i][j]); }
    solve(N, move(D));
}
#else
void solve() {
