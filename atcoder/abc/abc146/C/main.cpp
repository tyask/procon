...
void solve(long long A, long long B, long long X) {
    // ll left  = 0;
    // ll right = powint(10, 9) + 1;
    // ll ans = 0;
    // while (left < right - 1) {
    //     ll mid = (left + right) / 2;
    //     ll p = A * mid + B * ((int) log10(mid) + 1);
    //     // out(left, right, mid, p, X);
    //     if (p <= X) {
    //         left = mid;
    //         ans = mid;
    //     } else {
    ...
    vec<ll> v(powint(10, 9));
    rep(powint(10, 9)) v[i] = i+1;
    auto it = lower_bound(rng(v), X, [&](auto a, auto b) {
        return A * a + B * ((int)log10(a) + 1) < A * b + B * ((int)log10(b) + 1);
    });
