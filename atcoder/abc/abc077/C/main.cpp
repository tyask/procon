...
#if __ATCODER__ == 1
void solve2(long long N, std::vector<long long> A, std::vector<long long> B, std::vector<long long> C) {
    sort(A);
    sort(B);
    sort(C);
    ...
    ll ans = 0;
    rep(i, N) {
        ll na = distance(A.begin(), lower_bound(rng(A), B[i]));
        ll nc = distance(upper_bound(rng(C), B[i]), C.end());
        ans += na * nc;
    }
...
void solve(long long N, std::vector<long long> A, std::vector<long long> B, std::vector<long long> C) {
    ...
    vec<ll> bb(N);
    rep(i, N) {
        auto it = upper_bound(rng(C), B[i]);
        bb[i] = distance(it, C.end());
    }
    ...
    rep(i, N) {
        ll a = A[i];
        auto bit = upper_bound(rng(B), A[i]);
        if (bit == B.end()) continue;
        int bpos = distance(B.begin(), bit);
        ans += bb[bpos];
    }
...
void solve() {
    ...
    solve2(N, std::move(A), std::move(B), std::move(C));
}
#else
void solve() {
}
