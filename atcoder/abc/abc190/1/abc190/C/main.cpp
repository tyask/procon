...
void solve(ll N, ll M, vec<ll> A, vec<ll> B, ll K, vec<ll> C, vec<ll> D) {
    decrements(A,B,C,D);
    vvec<ll> bx{C,D};
    ll ans=0;
    rep(b,1<<K) {
        ll ax=0;
        vec<ll> a(N);
        rep(i,K) a[bx[b>>i&1][i]]++;
        rep(i,M) if (a[A[i]] && a[B[i]]) ax++;
        chmax(ans, ax);
    }
    out(ans);
}
...
void solve() {
    ...
    vec<ll> C(K);
    vec<ll> D(K);
    rep(i,K) { input(C[i]); input(D[i]); }
    solve(N, M, move(A), move(B), K, move(C), move(D));
}
#else
void solve() {
}
