...
void solve(ll N, vec<ll> C, vec<ll> P, ll Q, vec<ll> L, vec<ll> R) {
    rep(N) C[i]--;
    rep(Q) L[i]--, R[i]--;
    vvec<ll> p(2, vec<ll>(N));
    rep(N) p[C[i]][i] = P[i];

    cumsum<ll> s1(p[0]), s2(p[1]);
    rep(Q) out(s1(L[i],R[i]+1), s2(L[i],R[i]+1));
}
...
void solve() {
    ...
    vec<ll> C(N);
    vec<ll> P(N);
    rep(i, N) { in(C[i]); in(P[i]); }
    LL(Q);
    vec<ll> L(Q);
    vec<ll> R(Q);
    rep(i, Q) { in(L[i]); in(R[i]); }
    solve(N, move(C), move(P), Q, move(L), move(R));
}
#else
void solve() {
}
