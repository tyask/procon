...
void solve(ll N, vec<ll> lx, vec<ll> ly, vec<ll> rx, vec<ll> ry) {
    const int M = 1001;
    vvec<ll> g(M, vec<ll>(M+1));
    rep(n,N) {
        auto [x1,y1,x2,y2] = tuple{lx[n],ly[n],rx[n],ry[n]};
        rep(y,y1,y2) g[y][x1]++, g[y][x2]--;
    }
    ...
    rep(i,1,N+1) out(ans[i]);

}

void solve2(ll N, vec<ll> lx, vec<ll> ly, vec<ll> rx, vec<ll> ry) {
    const int M = 1001;
    vvec<ll> g(M, vec<ll>(M+1));
    rep(n,N) {
        auto [x1,y1,x2,y2] = tuple{lx[n],ly[n],rx[n],ry[n]};
    ...
    rep(i,M)rep(j,M) g[i][j+1] += g[i][j];
    rep(i,M-1)rep(j,M) g[i+1][j] += g[i][j];

    vec<ll> ans(N+1);
    rep(i,M)rep(j,M) ans[g[i][j]]++;
...
void solve() {
    ...
    vec<ll> rx(N);
    vec<ll> ry(N);
    rep(i, N) { in(lx[i]); in(ly[i]); in(rx[i]); in(ry[i]); }
    solve2(N, move(lx), move(ly), move(rx), move(ry));
}
#else
void solve() {
}
