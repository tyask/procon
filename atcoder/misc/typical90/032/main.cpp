...
void solve(ll N, vec<vec<ll>> A, ll M, vec<ll> X, vec<ll> Y) {
    vec<int> h(N);
    iota(rng(h), 1);
    set<pll> m;
    rep(M) m.emplace(X[i], Y[i]), m.emplace(Y[i], X[i]);

    ll ans = inf<ll>;
    do {
        bool ok = true;
        rep(N-1) if (m.count({h[i], h[i+1]})) ok = false;
        if (!ok) continue;
        ll a = 0;
        rep(N) a += A[h[i]-1][i];
        chmin(ans, a);
    } while (next_permutation(rng(h)));
...
void solve() {
    ...
    rep(i, N) rep(j, N) { in(A[i][j]); }
    LL(M);
    vec<ll> X(M);
    vec<ll> Y(M);
    rep(i, M) { in(X[i]); in(Y[i]); }
    solve(N, move(A), M, move(X), move(Y));
}
#else
void solve() {
}
