...
void solve() {
    ...
    unionfind u(H*W);
    auto enc = [&](ll r, ll c) { return r*W+c;};
    rep(q,Q) {
        LL(t);
        if (t==1) {
            LL(r,c);
            r--, c--;
            rep(4) {
                ll ri = r+dy[i], ci = c+dx[i];
                if(0<=ri&&ri<H&&0<=ci&&ci<W&&g[ri][ci]) {
                    u.unite(enc(ri,ci), enc(r, c));
                }
            }
            g[r][c] = 1;
        } else {
            LL(ra, ca, rb, cb);
            ra--, ca--, rb--, cb--;
            Yes(g[ra][ca]&&g[rb][cb]&&u.same(enc(ra,ca), enc(rb,cb)));
        }
    }
