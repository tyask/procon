...
void solve(ll N, ll Q, vec<ll> X, vec<ll> Y, vec<ll> Z, vec<ll> W) {
    rep(Q) X[i]--, Y[i]--, Z[i]--;
    ...
    rep(k,60) {
        ...
        rep(b,1<<N) {
            ...
            rep(i,Q) {
                ...
                if (((b>>x&1)|(b>>y&1)|(b>>z&1)) != (w>>k&1)) ok=false;
            }
            if (ok) cnt++;
        }
        ans*=cnt;
...
void solve() {
    ...
    vec<ll> z(Q);
    vec<ll> w(Q);
    rep(i, Q) { in(x[i]); in(y[i]); in(z[i]); in(w[i]); }
    solve(N, Q, move(x), move(y), move(z), move(w));
}
#else
void solve() {
}
