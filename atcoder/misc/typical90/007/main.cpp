...
void solve(ll N, vec<ll> A, ll Q, vec<ll> B) {
    sort(A);
    rep(Q) {
        int idx = lower_bound(rng(A), B[i]) - A.begin();
        ll a = inf<ll>;
        if (idx<N) chmin(a, abs(A[idx]-B[i]));
        if (0<idx) chmin(a, abs(A[idx-1]-B[i]));
        out(a);
    }
}
