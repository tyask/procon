...
void solve(ll N, ll M, vec<ll> A, vec<ll> B, vec<ll> C) {
    rep(M) A[i]--, B[i]--;
    dijkstra d1(N), dn(N);
    rep(M) {
        d1.add(A[i], {(int)B[i], C[i]});
        d1.add(B[i], {(int)A[i], C[i]});
        dn.add(A[i], {(int)B[i], C[i]});
        dn.add(B[i], {(int)A[i], C[i]});
    }

    d1.run(0);
    dn.run(N-1);
    rep(k,N) out(d1.d[k]+dn.d[k]);
