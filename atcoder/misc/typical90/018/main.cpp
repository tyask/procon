...
void solve(ll T, ll L, ll X, ll Y, ll Q, vec<ll> E) {
    using ld = long double;
    ld dt = -2*PI/T;
    ld r = L/2.0;
    rep(Q) {
        ...
        ld y = -sin(th)*(-r);
        ld z = cos(th)*(-r)+r;
        auto [a1,a2,a3] = tuple{x-X,y-Y,z-0};
        auto [b1,b2,b3] = tuple{x-X,y-Y,0-0};
        ld cos = (a1*b1+a2*b2+a3*b3)/(sqrt(a1*a1+a2*a2+a3*a3)*sqrt(b1*b1+b2*b2+b3*b3));
        out(acosf64(cos)*180/PI);
    }
}

void solve2(ll T, ll L, ll X, ll Y, ll Q, vec<ll> E) {
    using ld = long double;
    ld dt = -2*PI/T;
    ld r = L/2.0;
    rep(Q) {
        ...
        ld y = -sin(th)*(-r);
        ld z = cos(th)*(-r)+r;
        ld a = atan2(z,sqrt(pow(x-X,2)+pow(y-Y,2)));
        out(a*180/PI);
    }
}
