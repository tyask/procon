...
void solve(str s, str t) {
    str ss = s, tt = t;
    transform(rng(ss), ss.begin(), [](auto c){return tolower(c);});
    transform(rng(tt), tt.begin(), [](auto c){return tolower(c);});

    if (s==t) out("same");
    else if (ss==tt) out("case-insensitive");
    else out("different");
}
