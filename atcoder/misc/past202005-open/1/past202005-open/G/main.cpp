...
void solve(ll N, ll X, ll Y, vec<ll> x, vec<ll> y) {
    const ll M = 403;
    const ll A = 201;
    X+=A, Y+=A;
    rep(N) x[i]+=A, y[i]+=A;

    const vec<ll> dx{1,0,-1,1,-1, 0};
    const vec<ll> dy{1,1, 1,0, 0,-1};

    vvec<ll> g(M, vec<ll>(M, inf<ll>));
    rep(N) g[y[i]][x[i]] = -1;
    ...
    while (q.size()) {
        ...
        rep(k,6) {
            ...
            if (in(g,ni,nj)&&g[ni][nj]!=-1&&chmin(g[ni][nj],g[i][j]+1)) {
                q.emplace(ni,nj);
            }
        }
    }

    if (g[Y][X]==inf<ll>) out(-1);
    else out(g[Y][X]);
}
