...
void solve(ll N, vec<str> a) {
    vec<set<char>> cc(N);
    rep(N) each(c, a[i]) cc[i].insert(c);

    deque<char> q;
    if (N%2==1) q.push_back(a[N/2][0]);

    rrep(N/2) {
        erep(c,'a','z') {
            if (cc[i].count(c) && cc[N-i-1].count(c)) {
                q.push_front(c);
                q.push_back(c);
                break;
            }
    ...
    if (q.size()<N) out(-1).exit();

    str ans;
    while (q.size()) ans += q.front(), q.pop_front();
    out(ans);
}
