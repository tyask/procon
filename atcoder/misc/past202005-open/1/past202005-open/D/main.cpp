...
void solve(ll N, vec<str> s) {
    auto empty = [&](initializer_list<char> list) {
        return all_of(rng(list), [](auto c){return c=='.';});
    };

    str a;
    rep(j,1,4*N+1,4) {
        if (empty({s[0][j]})) a+='1';
        else if (empty({s[0][j+1]})) a+='4';
        else if (empty({s[1][j+1],s[2][j+1],s[3][j+1]})) {
            if (empty({s[4][j+1]})) a+='7'; else a+='0';
        }
        else if (empty({s[1][j],s[1][j+1],s[3][j+1],s[3][j+2]})) a+='2';
        else if (empty({s[1][j],s[1][j+1],s[3][j],s[3][j+1]})) a+='3';
        else if (empty({s[1][j+1],s[1][j+2],s[3][j],s[3][j+1]})) a+='5';
        else if (empty({s[1][j+1],s[1][j+2],s[3][j+1]})) a+='6';
        else if (empty({s[1][j+1],s[3][j],s[3][j+1]})) a+='9';
        else a+='8';
    }
    out(a);
}
