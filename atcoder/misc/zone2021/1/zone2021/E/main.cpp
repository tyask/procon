...
void solve(ll R, ll C, vec<vec<ll>> A, vec<vec<ll>> B) {
}

void solve() {
    LL(R);
    LL(C);
    vec<vec<ll>> A(R, vec<ll>(C-1));
    rep(i, R) rep(j, C-1) { input(A[i][j]); }
    vec<vec<ll>> B(R-1, vec<ll>(C));
    rep(i, R-1) rep(j, C) { input(B[i][j]); }
    solve(R, C, move(A), move(B));
}
#else
void solve() {
}
