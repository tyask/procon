...
void solve(str S) {
    ...
    rep(S.size()) {
        if (S[i]=='R') r = !r;
        else if (r) q.push_front(S[i]);
        else q.push_back(S[i]);
    }

    deque<char> q2;
    if (r) while (q.size()) q2.push_back(q.back()), q.pop_back();
    else while (q.size()) q2.push_back(q.front()), q.pop_front();

    deque<char> q3;
    while (q2.size()) {
        char c = pop_front(q2);
        if (q3.empty() || q3.back() != c) q3.push_back(c);
        else q3.pop_back();
    }

    str t;
    while (q3.size()) t += pop_front(q3);
