...
void solve(ll N, vec<ll> A, vec<ll> B, vec<ll> C, vec<ll> D, vec<ll> E) {
}
...
void solve() {
    ...
    vec<ll> D(N);
    vec<ll> E(N);
    rep(i, N) { input(A[i]); input(B[i]); input(C[i]); input(D[i]); input(E[i]); }
    solve(N, move(A), move(B), move(C), move(D), move(E));
}
#else
void solve() {
}
