...
TEMPLATE(Cont) void sort(Cont& c) { sort(rng(c)); }
TEMPLATE(Cont, Comp) void sort(Cont& c, Comp comp) { sort(rng(c), comp); }
TEMPLATE(Cont) void reverse(Cont& c) { reverse(rng(c)); }
...
TEMPLATE(T)    void uniq(vec<T>& v) { set<T> s(rng(v)); v = vec<T>(rng(s)); }
TEMPLATE(Cont) size_t sz(const Cont& c) { return c.size(); }
TEMPLATE(T)    void compress(vec<T>& v) { auto a = v; uniq(a); sort(a); rep(sz(v)) v[i] = lower_bound(rng(a),v[i])-a.begin(); }
...
void solve(ll T, vec<ll> N, vec<ll> M, vec<ll> A, vec<ll> B) {
}
...
void solve() {
    ...
    vec<ll> A(T);
    vec<ll> B(T);
    rep(i, T) { input(N[i]); input(M[i]); input(A[i]); input(B[i]); }
    solve(T, move(N), move(M), move(A), move(B));
}
#else
void solve() {
}
