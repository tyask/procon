...
void solve(ll H, ll W, vec<vec<ll>> A) {
    const int s  = (H-1)*W;
    const int d1 = (H-1)*W+(W-1);
    const int d2 = W-1;
    ...
    rep(i,H)rep(j,W) {
        rep(k,4) {
            ...
            di1.add(i*W+j, {int(ni*W+nj), a[ni][nj]});
        }
    }

    di1.run(s);
    ll c1 = di1.d[d1];
    each(n, di1.shortest_path(d1)) a[n/W][n%W] = 0;
    ...
    rep(i,H)rep(j,W) {
        rep(k,4) {
            int ni = i+dy[k], nj = j+dx[k];
            if (ni<0||H<=ni||nj<0||W<=nj) continue;
            di2.add(i*W+j, {int(ni*W+nj), a[ni][nj]});
        }
    }
    ...
    ll ans = c1 + c2;
    out(ans);
    each(n, di2.shortest_path(d2)) a[n/W][n%W] = 0;

    cout << fixed << setw(5);
    rep(i,H) { rep(j,W) cout << setw(5) << A[i][j]; cout << endl; }
    out("***");
    rep(i,H) { rep(j,W) cout << setw(5) << a[i][j]; cout << endl; }
}
