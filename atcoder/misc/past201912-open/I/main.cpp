...
void solve(ll N, ll M, vec<str> S, vec<ll> C) {
    vec<int> bs(M);
    rep(i,M)rep(j,N) bs[i] = (bs[i]<<1) | (S[i][j]=='Y');

    vvec<ll> dp(M+1, vec<ll>(1<<N, LINF));
    dp[0][0] = 0;
    rep(i, M) rep(j, 1<<N) {
        chmin(dp[i+1][j],       dp[i][j]);
        chmin(dp[i+1][j|bs[i]], dp[i][j]+C[i]);
    }
