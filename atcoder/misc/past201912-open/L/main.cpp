...
void solve(ll N, ll M, vec<ll> x, vec<ll> y, vec<ll> c, vec<ll> X, vec<ll> Y, vec<ll> C) {
}
...
void solve() {
    ...
    vec<ll> Y(M);
    vec<ll> C(M);
    rep(i, M) { in(X[i]); in(Y[i]); in(C[i]); }
    solve(N, M, move(x), move(y), move(c), move(X), move(Y), move(C));
}
#else
void solve() {
}
