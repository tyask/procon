...
void solve(ll N, ll X, vec<ll> a, vec<ll> b, vec<ll> c) {
    decrements(a, b);
    vvec<ll> g(N, vec<ll>(N));
    vec<ll> gc(N);
    rep(N-1) g[a[i]][b[i]]=g[b[i]][a[i]]=c[i], gc[a[i]]++, gc[b[i]]++;

    vec<ll> vis(N);
    auto dfs = [&](auto self, ll v, ll s) {
        if (s == X) return true;
        if (vis[v]) return false;
        vis[v] = 1;

        rep(N) if (!vis[i] && g[v][i]) {
            if (self(self, i, s+g[v][i])) return true;
        }
        return false;
    };
...
void solve() {
    ...
    vec<ll> b(N-1);
    vec<ll> c(N-1);
    rep(i, N-1) { input(a[i]); input(b[i]); input(c[i]); }
    solve(N, X, move(a), move(b), move(c));
}
#else
void solve() {
}
