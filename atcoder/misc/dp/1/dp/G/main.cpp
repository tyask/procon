...
void solve(ll N, ll M, vec<ll> x, vec<ll> y) {
    decrements(x,y);
    vec<ll> dp(N);
    auto h = graph(N).digraph(y,x);
    auto dfs = [&](auto self, ll v) -> ll {
        if (dp[v]) return dp[v];
        ll mx=0;
        each(n,h[v]) chmax(mx,self(self,n)+1);
        return dp[v]=mx;
    };
    ll ans=0;
    rep(N) chmax(ans,dfs(dfs,i));
    out(ans);
}
...
void solve2(ll N, ll M, vec<ll> x, vec<ll> y) {
    decrements(x,y);
    vec<ll> dp(N);
    auto g = graph(N).digraph(x,y);
    auto s = toporogical_sort(g);
    each(v,s) each(n,g[v]) chmax(cp[n],dp[v]+1);
    out(max(dp));
}
...
void solve() {
    ...
    vec<ll> x(M);
    vec<ll> y(M);
    rep(i,M) { input(x[i]); input(y[i]); }
    solve2(N, M, move(x), move(y));
}
#else
void solve() {
}
