...
TEMPLATE(T)    ostream& operator<<(ostream& os, const vvec<T>& c) { each(v, c) out(v); return os; }
...
// 貰うDP
void solve2(long long N, long long W, std::vector<long long> w, std::vector<long long> v) {
    vv(ll, dp, N+1, W+1);
    erep(i, 1, N) erep(j, W) {
...
void solve() {
    ...
    solve2(N, W, std::move(w), std::move(v));
}
#else
void solve() {
}
