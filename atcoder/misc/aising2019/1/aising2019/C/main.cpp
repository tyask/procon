...
void solve(ll H, ll W, vec<str> S) {
    vvec<int> g(H, vec<int>(W, -1));
    ...
    while (q.size()) {
        auto [ci,cj,x] = pop(q);
        if (!chmax(g[ci][cj], x)) continue;
        rep(k,2) {
            int ni = ci+dy[k], nj = cj+dx[k];
            if (!in(S, ni, nj)) continue;
            if (S[ci][cj]!=S[ni][nj]) q.emplace(ni,nj,x+1);
            else q.emplace(ni,nj,x);
        }
    }
