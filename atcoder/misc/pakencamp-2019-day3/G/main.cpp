...
#if __ATCODER__ == 1
void solve(long long N, std::vector<long long> A, long long M, std::vector<long long> B, long long Q, std::vector<long long> T, std::vector<long long> C, std::vector<long long> D) {
}
#else
void solve() {
}
...
int main() {
    ...
    solve(N, std::move(A), M, std::move(B), Q, std::move(T), std::move(C), std::move(D));
#elif __DEBUG__
    rep(10) {
        solve();
        out("*****");
