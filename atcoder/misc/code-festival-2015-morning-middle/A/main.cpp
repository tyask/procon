...
#if __ATCODER__ == 1
void solve(long long N, long long K, long long M, long long R, std::vector<long long> S) {
    sort(S, greater<ll>());
    if (K < N && sum(vec<ll>(rng(S, K))) >= R*K) {
        out(0);
        return;
    }

    ll m = R*K-sum(vec<ll>(rng(S, K-1)));
    if (m > M) out(-1);
    else if (m < 0) out(0);
    else out(m);
}
