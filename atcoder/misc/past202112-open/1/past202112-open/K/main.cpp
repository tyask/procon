...
void solve(ll N, ll M, ll Q, ll K, vec<ll> a, vec<ll> u, vec<ll> v, vec<ll> s, vec<ll> t) {
}
...
void solve() {
    ...
    vec<ll> s(Q);
    vec<ll> t(Q);
    rep(i, Q) { input(s[i]); input(t[i]); }
    solve(N, M, Q, K, move(a), move(u), move(v), move(s), move(t));
}
#else
void solve() {
}
