...
template<typename T=ll> struct seg_tree {
    using func = function<T(T,T)>;
    ll n;
    vec<T> dat;
    func fx;
    T e;
    seg_tree(ll n, func fx, T e) : n(pow2(n)), dat(n*4, e), fx(fx), e(e) {}
...
struct RMQ : seg_tree<ll> {
    RMQ(ll n) : seg_tree<ll>(n, [](ll x, ll y){return min(x,y);}, inf<ll>) {}
};
...
void solve(ll N, ll Q, vec<ll> A, vec<ll> T, vec<ll> X, vec<ll> Y) {
    ...
    RMQ rmq(N);
    map<ll,set<ll>> m;
    rep(N) rmq.update(i, A[i]);
    rep(N) m[A[i]].insert(i);
...
void solve() {
    ...
    vec<ll> X(Q);
    vec<ll> Y(Q);
    rep(i, Q) { input(T[i]); input(X[i]); input(Y[i]); }
    solve(N, Q, move(A), move(T), move(X), move(Y));
}
#else
void solve() {
}
