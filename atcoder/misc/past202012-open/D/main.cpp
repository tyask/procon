...
#if __ATCODER__ == 1
int leading_zeros(const string& s) {
    int ret = 0;
    rep(s.size()) {
        if (s[i]!='0') {
...
void solve(long long N, std::vector<std::string> S) {
    sort(S, [](string& s1, auto& s2){
        int nz1 = distance(s1.begin(), find_if(rng(s1), [](char c){ return c != '0';}));
        int nz2 = distance(s2.begin(), find_if(rng(s2), [](char c){ return c != '0';}));

        string v1 = s1.substr(nz1);
        string v2 = s2.substr(nz2);
