...
#if __ATCODER__ == 1
void solve(long long N, long long M, std::vector<long long> a, std::vector<long long> b, long long Q, std::vector<long long> T, std::vector<long long> x) {
}
...
void solve() {
    ...
    solve(N, M, std::move(a), std::move(b), Q, std::move(T), std::move(x));
}
#else
void solve() {
}
