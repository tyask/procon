...
void solve() {
    ...
    rep(b, 1<<N) {
        set<int> s;
        rep(i,N) if (b>>i&1) s.insert(i+1);
        bool ok = true;
        rep(M) if (s.count(A[i])>0&&s.count(B[i])>0&&s.count(C[i])>0) ok = false;
        if (!ok) continue;

        set<int> a;
        rep(M) {
            if (s.count(A[i])>0&&s.count(B[i])>0&&s.count(C[i])==0) a.insert(C[i]);
            if (s.count(A[i])>0&&s.count(C[i])>0&&s.count(B[i])==0) a.insert(B[i]);
            if (s.count(B[i])>0&&s.count(C[i])>0&&s.count(A[i])==0) a.insert(A[i]);
        }
