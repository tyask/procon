...
void solve() {
    ...
    vec<pii> hs;
    rep(i,H)rep(j,W)if(S[i][j]=='#') hs.emplace_back(i,j);
    int k = hs.size();

    vv(int, v, H, W);
    vec<pii> ans;
    auto dfs = [&](auto&& self, int i, int j, vec<pii> p){
        ...
        rep(k, 4) {
            ...
            if (self(self, ni, nj, p)) return true;
        }
    ...
    each(p, hs) {
        if (dfs(dfs, p.FI, p.SE, vec<pii>())) {
            out(k);
            each(pa, ans) out(pa.FI+1, pa.SE+1);
            break;
        }
    }
}
