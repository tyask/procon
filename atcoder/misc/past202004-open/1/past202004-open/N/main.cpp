...
void solve(ll N, ll Q, vec<ll> xmin, vec<ll> ymin, vec<ll> D, vec<ll> C, vec<ll> A, vec<ll> B) {
}
...
void solve() {
    ...
    vec<ll> D(N);
    vec<ll> C(N);
    rep(i, N) { input(xmin[i]); input(ymin[i]); input(D[i]); input(C[i]); }
    vec<ll> A(Q);
    vec<ll> B(Q);
    rep(i, Q) { input(A[i]); input(B[i]); }
    solve(N, Q, move(xmin), move(ymin), move(D), move(C), move(A), move(B));
}
#else
void solve() {
}
