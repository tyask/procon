...
void solve(str S) {
    const str T = "abcdefghijklmnopqrstuvwxyz.";
    const ll M = T.size();
    const ll N = S.size();
    vec<str> can;

    rep(i,M) can.PB({T[i]});
    rep(i,M) rep(j,M) can.PB({T[i],T[j]});
    rep(i,M) rep(j,M) rep(k,M) can.PB({T[i],T[j],T[k]});

    ll ans = 0;
    each(t, can) {
        rep(i,N) {
            if (i+t.size()>N) continue;
            bool ok = true;
            rep(j,t.size()) if (t[j]!='.' && t[j]!=S[i+j]) ok = false;
            if (ok) { ans++; break; }
        }
    }
