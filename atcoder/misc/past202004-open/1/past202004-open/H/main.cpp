...
void solve() {
    ...
    vvec<pll> v(10+1);
    rep(i,N) rep(j,M) if (isdigit((int)A[i][j])) v[A[i][j]-'0'].emplace_back(i,j);

    pll s, g;
    rep(i,N) rep(j,M) if (A[i][j]=='S') s = {i,j}; else if (A[i][j]=='G') g = {i,j};

    ll ans = inf<ll>;
    using tup = tuple<ll,ll,ll>;
    vvec<ll> m(N,vec<ll>(M, inf<ll>));
    queue<tup> q;
    ...
    while (q.size()) {
        ...
        char a = A[i][j];
        if (a == '9') {
            chmin(ans, c+abs(g.FI-i)+abs(g.SE-j));
            continue;
        }

        if (!chmin(m[i][j], c)) continue;

        int nt = (a=='S') ? 1 : a-'0'+1;
        each(n, v[nt]) {
            q.emplace(n.FI,n.SE,c+abs(n.FI-i)+abs(n.SE-j));
        }
    }
