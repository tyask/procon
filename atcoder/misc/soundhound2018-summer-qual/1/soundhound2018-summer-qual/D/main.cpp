...
void solve(ll n, ll m, ll s, ll t, vec<ll> u, vec<ll> v, vec<ll> a, vec<ll> b) {
}
...
void solve() {
    ...
    vec<ll> a(m);
    vec<ll> b(m);
    rep(i, m) { input(u[i]); input(v[i]); input(a[i]); input(b[i]); }
    solve(n, m, s, t, move(u), move(v), move(a), move(b));
}
#else
void solve() {
}
