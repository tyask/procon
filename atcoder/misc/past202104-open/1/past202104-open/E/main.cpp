...
void solve(ll N, str S) {
    ...
    rep(N) {
        ...
        if (c=='R') q.push_back(i+1);
        if ('A'<=c && c<='C') {
            int k = c - 'A' + 1;
            if (q.size()<=k-1) out("ERROR");
            else {
                deque<ll> q2;
                rep(k) q2.push_back(q.front()), q.pop_front();
                out(q2.back()); q2.pop_back();
                while(q2.size()) q.push_front(q2.back()), q2.pop_back();
            }
        }
        if ('D'<=c && c<='F') {
            int k = c - 'D' + 1;
            if (q.size()<=k-1) out("ERROR");
                ...
                deque<ll> q2;
                rep(k) q2.push_front(q.back()), q.pop_back();
                out(q2.front()); q2.pop_front();
                while(q2.size()) q.push_back(q2.front()), q2.pop_front();
            }
        }
    }
}
