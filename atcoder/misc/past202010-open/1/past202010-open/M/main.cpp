...
void solve(ll N, ll Q, vec<ll> a, vec<ll> b, vec<ll> u, vec<ll> v, vec<ll> c) {
    decrements(a,b,u,v);
    auto t = graph(N).undigraph(a, b);
    vec<ll> col(N,-1);
    rep(Q) col[u[i]] = col[v[i]] = c[i];

    map<pll,ll> col2;
    vec<ll> vis(N);
    rep(N) if (sz(t[i])==1) {
        auto dfs = [&](auto self, ll v, ll c) {
            if (vis[v]) return;
            vis[v]=1;

            if (c==col[v]) c = 0;
            each(n,t[v]) if (!vis[n]) {
                ll cn = col[n]>0 ? col[v] : c;
                col2[minmax(v,n)]=cn;
                out(minmax(v,n), cn);
                self(self,n,cn);
            }
    ...
    rep(N-1) out(col2[minmax(a[i],b[i])]);
}
...
void solve() {
    ...
    vec<ll> v(Q);
    vec<ll> c(Q);
    rep(i, Q) { input(u[i]); input(v[i]); input(c[i]); }
    solve(N, Q, move(a), move(b), move(u), move(v), move(c));
}
#else
void solve() {
}
