...
void solve() {
    ...
    vec<ll> w(N-1);
    rep(N-1) w[i] = ((i+1)&1) ? h[i]-h[i+1] : h[i+1]-h[i];
    ...
    ll x = 0;
    rep(Q) {
        auto [ti,ui,vi] = tuple(t[i],u[i],v[i]);
        if (ti==1) x+=vi;
        else if (ti==2) x-=vi;
        else {
            if ((ui+1)&1) {
                if (ui<N-1) m[w[ui]]--, w[ui]+=vi, m[w[ui]]++;
                if (ui>0) m[w[ui-1]]--, w[ui-1]+=vi, m[w[ui-1]]++;
            } else {
                if (ui<N-1) m[w[ui]]--, w[ui]-=vi, m[w[ui]]++;
                if (ui>0) m[w[ui-1]]--, w[ui-1]-=vi, m[w[ui-1]]++;
            }
