...
struct dijkstra {
    ...
    dijkstra& add(ll from, ll to, ll cost) {
        assert(0<=from && from<n && 0<=to && to<n);
        g[from].PB(edge{to, cost});
        return *this;
    }

    dijkstra& add2(ll p1, ll p2, ll cost) {
        return add(p1,p2,cost).add(p2,p1,cost);
    }
...
void solve() {
    ...
    vec<ll> A(M), B(M), C(M);
    rep(M) cin >> A[i] >> B[i] >> C[i];
    decrements(A,B);

    ll n = N;
    ll ax=n++, ay=n++, bx=n++, by=n++, cx=n++, cy=n++;

    dijkstra d(n);
    rep(M) d.add2(A[i],B[i],C[i]);
    ...
    rep(N) {
        if(S[i]=='A')      d.add(i,ax,0), d.add(ay,i,0);
        else if(S[i]=='B') d.add(i,bx,0), d.add(by,i,0);
        else               d.add(i,cx,0), d.add(cy,i,0);
    }

    out(d.run(0).mincost(N-1));
