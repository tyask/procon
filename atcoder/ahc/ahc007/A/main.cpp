...
struct kruskal {
    using ll = long long;
    struct edge {ll key; int u, v; ll cost; };
...
ll dist(ll u, ll v) {
    return round(sqrtl(powint(X[u]-X[v],2) + powint(Y[u]-Y[v],2)));
}
...
void solve2() {
    // weight=dの最小全域木を作って適当につなぐ
    kruskal k(N);
    rep(M) k.add({i, U[i], V[i], 3*dist(U[i], V[i])});
    map<ll, kruskal::edge> es = k.run();
...
void solve3() {
    ...
    rep(M) {
        LL(L);
        k.m[i].cost = L;
        map<ll, kruskal::edge> es2 = k.run();
        if (es.count(i) && es2.count(i)) out(1), es = es2;
        else if (es.count(i) && !es2.count(i)) out(0);
        else if (!es.count(i) && es2.count(i)) out(1), es = es2;
        else out(0);
        cout.flush();
    }
}
