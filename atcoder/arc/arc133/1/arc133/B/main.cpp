...
void solve(ll N, vec<ll> P, vec<ll> Q) {
    vvec<ll> m(N+1);
    rep(N) each(n,divisors(Q[i])) m[n].PB(i);

    map<pll,ll> mem;
    auto dfs = [&](auto self, ll pi, ll pos, ll cnt) -> ll {
        if (pi==N || pos==N) return cnt;
        if (mem[{pi,pos}]) return mem[{pi,pos}]+cnt;
        // out(pi,P[pi],pos,cnt);
        auto& vv=m[P[pi]];

        ll ret=cnt;
        auto it = lower_bound(rng(vv), pos+1);
        if (it!=vv.end()) chmax(ret, self(self, pi+1, *it, cnt+1));
        chmax(ret, self(self, pi+1, pos, cnt));
        return mem[{pi,pos}]=ret;
    };
