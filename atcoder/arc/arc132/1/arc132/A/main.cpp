...
void solve(ll n, vec<ll> R, vec<ll> C, ll q, vec<ll> r, vec<ll> c) {
    decrements(r,c);
    str ans;
    rep(i,q) {
        ll ri = R[r[i]], ci = C[c[i]];
        if (n-ri+1<=ci) ans.PB('#'); else ans.PB('.');
    }
    out(ans);
}
...
void solve() {
    ...
    vec<ll> r(q);
    vec<ll> c(q);
    rep(i, q) { input(r[i]); input(c[i]); }
    solve(n, move(R), move(C), q, move(r), move(c));
}
#else
void solve() {
}
