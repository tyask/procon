...
#if __ATCODER__ == 1
void solve(long long C, std::vector<long long> N, std::vector<long long> M, std::vector<long long> L) {
    vec<ll> ans{0, 0, 0};
    rep(C) {
        vec<ll> v{N[i],M[i],L[i]};
        sort(v, greater<ll>());
        rep(3) chmax(ans[i], v[i]);
    }
...
void solve() {
    long long C;
    scanf("%lld",&C);
    std::vector<long long> N(C-1-0+1);
    std::vector<long long> M(C-1-0+1);
    std::vector<long long> L(C-1-0+1);
    for(int i = 0 ; i < C-1-0+1 ; i++){
        scanf("%lld",&N[i]);
        scanf("%lld",&M[i]);
        scanf("%lld",&L[i]);
    }
    solve(C, std::move(N), std::move(M), std::move(L));
}
#else
void solve() {
}
