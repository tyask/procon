...
#if __ATCODER__ == 1
template<typename N> set<N> andset(const set<N>& a, const set<N>& b) {
    set<N> r;
    set_intersection(a.begin(), a.end(), b.begin(), b.end(), inserter(r, r.end()));
    return r;
}

template<typename N> set<N> unionset(const set<N>& a, const set<N>& b) {
    set<N> r;
    set_union(a.begin(), a.end(), b.begin(), b.end(), inserter(r, r.end()));
    return r;
}

void solve(long long N_A, long long N_B, std::vector<long long> A, std::vector<long long> B) {
    set<ll> sa(rng(A));
    set<ll> sb(rng(B));

    set<ll> a = andset(sa, sb);
    set<ll> b = unionset(sa, sb);
...
void solve() {
    ...
    long long N_B;
    scanf("%lld",&N_B);
    std::vector<long long> A(N_A);
    for(int i = 0 ; i < N_A ; i++){
        scanf("%lld",&A[i]);
    }
    std::vector<long long> B(N_B);
    for(int i = 0 ; i < N_B ; i++){
        scanf("%lld",&B[i]);
    }
    solve(N_A, N_B, std::move(A), std::move(B));
}
#else
void solve() {
}
