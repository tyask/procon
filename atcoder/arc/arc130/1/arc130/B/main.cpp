...
void solve(ll H, ll W, ll C, ll Q, vec<ll> t, vec<ll> n, vec<ll> c) {
    reverse(t);
    reverse(n);
    reverse(c);
    set<ll> hs, ws;
    ...
    ll ha = 0, wa = 0;
    rep(q,Q) {
        ll nx = n[q]-1, cx = c[q]-1;
        if (t[q]==1) {
            if (hs.count(nx)) continue;
            cols[cx] += W - wa;
            hs.insert(nx);
            ha++;
        } else {
            if (ws.count(nx)) continue;
            cols[cx] += H - ha;
            ws.insert(nx);
            wa++;
        }
...
void solve() {
    ...
    vec<ll> n(Q);
    vec<ll> c(Q);
    rep(i, Q) { input(t[i]); input(n[i]); input(c[i]); }
    solve(H, W, C, Q, move(t), move(n), move(c));
}
#else
void solve() {
}
