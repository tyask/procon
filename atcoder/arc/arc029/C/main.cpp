...
#if __ATCODER__ == 1
void solve(long long N, long long M, std::vector<long long> c, std::vector<long long> a, std::vector<long long> b, std::vector<long long> r) {
}
...
void solve() {
    ...
    solve(N, M, std::move(c), std::move(a), std::move(b), std::move(r));
}
#else
void solve() {
}
