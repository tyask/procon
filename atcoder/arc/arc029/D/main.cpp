...
#if __ATCODER__ == 1
void solve(long long N, std::vector<long long> s, std::vector<long long> a, std::vector<long long> b, long long M, std::vector<long long> t) {
}
...
void solve() {
    ...
    solve(N, std::move(s), std::move(a), std::move(b), M, std::move(t));
}
#else
void solve() {
}
