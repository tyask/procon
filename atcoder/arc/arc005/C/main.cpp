...
void solve() {
    ...
    pii s, g;
    rep(i,H)rep(j,W) if(c[i][j]=='s') s={i,j}; else if(c[i][j]=='g') g={i,j};
    ...
    while(!q.empty()) {
        ...
        rep(k, 4) {
            ...
            auto [di, dp] = d[ni][nj];
            if (di>=0&&p<=dp) continue;
            if (c[ni][nj]=='#') {
                if (p == 0) continue;
                else {
                    q.emplace(ni, nj, p-1);
                    d[ni][nj] = {d[i][j].FI+1, p-1};
                }
            } else {
                q.emplace(ni, nj, p);
                d[ni][nj] = {d[i][j].FI+1, p};
            }
        }
    }

    YES(d[g.FI][g.SE].FI>=0);
}
...
void solve3() {
    ...
    rep(i,H)rep(j,W) {
        rep(k,4) {
            int ni = i+dy[k], nj = j+dx[k];
            if (ni<0||H<=ni||nj<0||W<=nj) continue;
            if (c[ni][nj]=='#') d.add(i*W+j, {ni*W+nj, 1});
            else d.add(i*W+j, {ni*W+nj, 0});
        }
    }

    d.run(s.FI*W+s.SE);
    YES(d.d[g.FI*W+g.SE]<=2);
