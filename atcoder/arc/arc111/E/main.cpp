...
#if __ATCODER__ == 1
void solve(long long T, std::vector<long long> A, std::vector<long long> B, std::vector<long long> C, std::vector<long long> D) {
}
#else
void solve() {
}
...
int main() {
    ...
    std::vector<long long> C(T);
    std::vector<long long> D(T);
    for(int i = 0 ; i < T ; i++){
        scanf("%lld",&A[i]);
        scanf("%lld",&B[i]);
        scanf("%lld",&C[i]);
    ...
    solve(T, std::move(A), std::move(B), std::move(C), std::move(D));
#else
    rep(10) {
        solve();
        out("*****");
