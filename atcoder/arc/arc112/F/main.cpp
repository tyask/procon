...
#if __ATCODER__ == 1
void solve(long long n, long long m, std::vector<long long> c, std::vector<std::vector<long long>> s) {
}
...
void solve() {
    ...
    std::vector<std::vector<long long>> s(m, std::vector<long long>(n));
    for(int i = 0 ; i < m ; i++){
        for(int j = 0 ; j < n ; j++){
            scanf("%lld",&s[i][j]);
        }
    }
    solve(n, m, std::move(c), std::move(s));
}
#else
void solve() {
}
