...
#if __ATCODER__ == 1
void solve(long long N, long long M, std::vector<long long> A, std::vector<long long> B, std::vector<long long> C, long long K, std::vector<long long> X, std::vector<long long> Y, std::vector<long long> Z) {
}
...
void solve() {
    ...
    std::vector<long long> Z(K);
    for(int i = 0 ; i < K ; i++){
        scanf("%lld",&X[i]);
        scanf("%lld",&Y[i]);
        scanf("%lld",&Z[i]);
    }
    solve(N, M, std::move(A), std::move(B), std::move(C), K, std::move(X), std::move(Y), std::move(Z));
}
#else
void solve() {
}
