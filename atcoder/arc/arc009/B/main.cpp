...
#if __ATCODER__ == 1
void solve(std::vector<long long> b, long long N, std::vector<long long> a) {
    vec<int> m(10, 0);
    rep(10) m[b[i]] = i;
    vec<string> s(N);
    rep(N) s[i] = to_string(a[i]);
    sort(s, [&](auto& s1, auto& s2){
